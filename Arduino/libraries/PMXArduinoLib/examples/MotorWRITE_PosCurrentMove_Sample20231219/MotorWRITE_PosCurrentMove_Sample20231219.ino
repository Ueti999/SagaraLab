//
//  @file MotorWRITE_Double_Sample20231219.ino
...
//   
//   MotorWRITE_Double_Sample20231219.inoはPMXのMotorREADコマンド、MotorWRITEコマンドに関する関数を実行するサンプルコードです。
//   MotorWRITE_Double_Sample20231219.ino is a sample code that executes functions related to the MotorREAD and MotorWRITE functions of PMX.
//
...
byte controlMode = PMX::ControlMode::Position + PMX::ControlMode::Current;    // 位置制御モード値+電流制御モード値
//byte controlMode = 0x01 + 0x04  // 数値でも指定できます。
...
void loop() {
  ...
  long receiveData[8];
    ...
    // MotorWRITE関数で動作指令
  long pos = -32000;  // 目標位置
  long current = 500;    // 目標電流値
  int writeDataCount = 2;
  long writeDatas[writeDataCount] = {pos, current};
