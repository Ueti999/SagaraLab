//
//  @file MotorWRITE_Speed_Sample20231219.ino
...
//   
//   MotorWRITE_Speed_Sample20231219.inoはPMXのMotorREADコマンド、MotorWRITEコマンドに関する関数を実行するサンプルコードです。
//   MotorWRITE_Speed_Sample20231219.ino is a sample code that executes functions related to the MotorREAD and MotorWRITE functions of PMX.
//
...
byte controlMode = PMX::ControlMode::Speed;    // 速度制御モード値
//byte controlMode = 0x02  // 数値でも指定できます。

// 応答データを指定する
// 以下の各ビットを1にすると応答データとして指定できます
// Bit7:入力電圧 Bit6:CPU温度 Bit5:モータ温度 Bit4:PWM出力 Bit3:推定トルク Bit2:電流 Bit1:速度 Bit0:位置
// receiveMode = PMX.ReceiveDataOption.Speed      // 速度情報を返す
...
void loop() {
  ...
  long receiveData[8];
    ...
    // MotorWRITE関数で動作指令
  long speed = 1000;      // 目標速度
  int writeDataCount = 1;
  long writeDatas[writeDataCount] = {speed};
  ...
  long targetVal1 = 2000; // 目標速度
  flag = pmx.MotorWRITESingle(ServoID, targetVal1, receiveMode, receiveData, controlMode);
